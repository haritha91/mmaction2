...
def extract_frame(vid_item):
    ...
    if task == 'rgb':
        if args.use_opencv:
            ...
            for i, vr_frame in enumerate(vr):
                if vr_frame is not None:
                    ...
                    mmcv.imwrite(out_img,
                                 f'{out_full_path}/image_{i + 1:05d}.jpg')
                else:
                    warnings.warn(
                        'Length inconsistent!'
            ...
            if args.new_short == 0:
                cmd = osp.join(
                    f"~/app/bin/denseflow '{full_path}' -b=20 -s=0 -o='{out_full_path}'"
        ...
        if args.input_frames:
            if args.new_short == 0:
                cmd = osp.join(
                    f"~/app/bin/denseflow '{full_path}' -a={method} -b=20 -s=1 -o='{out_full_path}'"  # noqa: E501
